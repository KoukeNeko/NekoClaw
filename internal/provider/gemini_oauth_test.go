...
func TestCompleteOAuthContinuesWhenLoadCodeAssistUnavailable(t *testing.T) {
	...
	if credential.AccessToken != "access-1" {
		t.Fatalf("unexpected access token: %q", credential.AccessToken)
	}
	if credential.RefreshToken != "refresh-1" {
		t.Fatalf("unexpected refresh token: %q", credential.RefreshToken)
	}
	if credential.Email != "tester@example.com" {
		t.Fatalf("unexpected email: %q", credential.Email)
	}
