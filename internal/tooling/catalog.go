...
func builtinToolCatalogSpecs() []builtinToolCatalogSpec {
	...
	return []builtinToolCatalogSpec{
		...
		{Definition: newToolDefinition("exec_command", "Execute an allowlisted command.", `{"type":"object","required":["argv"],"properties":{"argv":{"type":"array","items":{"type":"string"}},"workdir":{"type":"string"},"timeout_sec":{"type":"integer","minimum":1}}}`), ReadOnlySafe: false},
		{Definition: newToolDefinition("file_write", "Write file content.", `{"type":"object","required":["path","content"],"properties":{"path":{"type":"string"},"content":{"type":"string"},"mode":{"type":"string","enum":["overwrite","append"]}}}`), Mutating: true, ReadOnlySafe: false, RequiresApproval: true},
		{Definition: newToolDefinition("file_replace", "Replace text in a file.", `{"type":"object","required":["path","old","new"],"properties":{"path":{"type":"string"},"old":{"type":"string"},"new":{"type":"string"},"replace_all":{"type":"boolean"}}}`), Mutating: true, ReadOnlySafe: false, RequiresApproval: true},
		{Definition: newToolDefinition("git_add", "Run git add.", `{"type":"object","required":["pathspecs"],"properties":{"pathspecs":{"type":"array","items":{"type":"string"}}}}`), Mutating: true, ReadOnlySafe: false, RequiresApproval: true},
		{Definition: newToolDefinition("git_restore", "Run git restore.", `{"type":"object","required":["pathspecs"],"properties":{"pathspecs":{"type":"array","items":{"type":"string"}},"staged":{"type":"boolean"}}}`), Mutating: true, ReadOnlySafe: false, RequiresApproval: true},
		{Definition: newToolDefinition("git_commit", "Run git commit.", `{"type":"object","required":["message"],"properties":{"message":{"type":"string"}}}`), Mutating: true, ReadOnlySafe: false, RequiresApproval: true},
...
func builtinRuntimeSpecs(cfg ExecutorConfig) map[string]ToolSpec {
	...
	for _, spec := range builtinToolCatalogSpecs() {
		...
		readOnlySafe := effectiveReadOnlySafe(spec)
		requiresApproval := effectiveRequiresApproval(spec)
...
func effectiveReadOnlySafe(spec builtinToolCatalogSpec) bool {
	switch spec.Definition.Name {
	case "exec_command", "memory_save", "task_update":
		return false
	}
	if spec.ReadOnlySafe {
		return true
	}
	return !spec.Mutating && !effectiveRequiresApproval(spec)
}

func effectiveRequiresApproval(spec builtinToolCatalogSpec) bool {
	switch spec.Definition.Name {
	case "exec_command", "memory_save", "task_update":
		return false
	}
